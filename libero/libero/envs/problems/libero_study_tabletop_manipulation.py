...
@register_problem
class Libero_Study_Tabletop_Manipulation_Light(Libero_Study_Tabletop_Manipulation):
    def __init__(self, bddl_file_name, *args, **kwargs):
        self.workspace_name = "study_table"
        self.visualization_sites_list = []
        if "table_full_size" in kwargs:
        ...
        if "scene_xml" not in kwargs or kwargs["scene_xml"] is None:
            kwargs.update({"scene_xml": "scenes/libero_study_light_style.xml"})
        if "scene_properties" not in kwargs or kwargs["scene_properties"] is None:
            kwargs.update(
                {
                    "scene_properties": {
...
@register_problem
class Libero_Study_Tabletop_Manipulation_Texture(Libero_Study_Tabletop_Manipulation):
    def __init__(self, bddl_file_name, *args, **kwargs):
        self.workspace_name = "study_table"
        self.visualization_sites_list = []
        if "table_full_size" in kwargs:
        ...
        if "scene_xml" not in kwargs or kwargs["scene_xml"] is None:
            kwargs.update({"scene_xml": "scenes/libero_study_texture_style.xml"})
        if "scene_properties" not in kwargs or kwargs["scene_properties"] is None:
            kwargs.update(
                {
                    "scene_properties": {
